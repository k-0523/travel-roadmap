...
const Destination: React.VFC<{index: number, day: number}> = ({index, day}) => {
    ...
    const [destinationDisplayList, setDestinationDisplayList] = useState<{isDisplay: boolean}[]>([]);
    ...
    useEffect(() => {
        ...
        state.schedules[index].scheduleDetails.map((_) => {
            displays.push({isDisplay: true})
        });
        setDestinationDisplayList(displays);
    },[]);

    return useMemo(() => {
        return (
            <Box sx={{ px: 3, py:2}}>
                {state.schedules[index].scheduleDetails.length === destinationDisplayList.length && (
                    <>
    ...
    }, [dispatch, destinationDisplayList, update]);
}
