...
const Image: React.VFC<{index: number, detailIndex: number}> = ({index, detailIndex}) => {
    ...
    const [imagePathList, setImagePathList] = useState<{imagePath: string, isUploadNow: boolean}[]>([
        {
            imagePath: state.schedules[index].scheduleDetails[detailIndex].scheduleDetailImages[0]?.signedUrl,
            isUploadNow: false
        },
        {
            imagePath: state.schedules[index].scheduleDetails[detailIndex].scheduleDetailImages[1]?.signedUrl,
            isUploadNow: false
        },
        {
            imagePath: state.schedules[index].scheduleDetails[detailIndex].scheduleDetailImages[2]?.signedUrl,
            isUploadNow: false
        }
    ]);
